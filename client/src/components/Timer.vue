...
// Initialize from the store's current time remaining
...
const startTimer = () => {
  ...
  timerInterval = setInterval(() => {
    if (timeLeft.value > 0) {
      timeLeft.value--
      examStore.timeRemaining = timeLeft.value // Sync back to store for persistence
    } else {
...
watch(() => props.totalSeconds, (newVal) => {
  // Only override if the exam is entirely reset (timeRemaining matches totalSeconds)
...
const currentColor = computed(() => {
  if (timeLeft.value > 600) return 'var(--primary)' // > 10 min
  if (timeLeft.value > 300) return '#f59e0b' // 5-10 min (orange)
  return '#ef4444' // < 5 min (red)
})
</script>
